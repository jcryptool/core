...
public class Backend {
    ...
    public void btnSaveClick() {
        if (modus == 1) { // set
            ...
            visual.MsgBox(Messages.Backend_PopupErrorHeading, Messages.Backend_PopupErrorMessage, SWT.ICON_ERROR
                    | SWT.OK);
        }
    }

    /**
     * Checks if the inputed pattern is valid. If so it is in case of the <b>first input</b>
     * temporary saved. In case of a <b>matching confirmation input</b> it is saved and the mode is
     * changed to <i>check</i>. In case of a <b>not matching confirmation input</b> the input is
     * reseted and an information message is displayed in the status text.
     */
    private void setPattern() {
        ...
        if (isGreatEnough()) {
            ...
            btnCancelClick();
            visual.MsgBox(Messages.Backend_PopupInvalidHeading, Messages.Backend_PopupInvalidMessage, SWT.ICON_ERROR
                    | SWT.OK);
        }
    }
    ...
    public void recalculateLines() {
        ...
        // points[i][0] = visual.getCntrBtn()[lines[i][0]-1].getLocation().x +
        // visual.getCntrBtn()[lines[i][0]-1].getSize().x / 2;
        // points[i][1] = visual.getCntrBtn()[lines[i][0]-1].getLocation().y +
        // visual.getCntrBtn()[lines[i][0]-1].getSize().y / 2;
        // points[i][2] = visual.getCntrBtn()[lines[i][1]-1].getLocation().x +
        // visual.getCntrBtn()[lines[i][1]-1].getSize().x / 2;
        // points[i][3] = visual.getCntrBtn()[lines[i][1]-1].getLocation().y +
        // visual.getCntrBtn()[lines[i][1]-1].getSize().y / 2;
        // }
        // }
