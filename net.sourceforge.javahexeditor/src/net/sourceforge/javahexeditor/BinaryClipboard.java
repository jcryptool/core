...
public class BinaryClipboard {
    ...
    static final long maxClipboardDataInMemory = 4 * 1024 * 1024; // 4 Megs for byte[], 4 Megs for
                                                                  // text
    ...
    public void setContents(BinaryContent content, long start, long length) {
        ...
        try {
            if (length <= maxClipboardDataInMemory) {
                ...
                transfers = new Transfer[] {MemoryByteArrayTransfer.getInstance(),
    ...
    /*
     * The file is being reference counted. It will be deleted as soon as no javahexeditor process
	...
	long tryGettingFileByteArray(BinaryContent content, long start, boolean insert) {
        ...
        if (clipboardFile.equals(lastPaste)) {
            ...
            myClipboard.setContents(new Object[] {lastPaste}, new Transfer[] {FileByteArrayTransfer
                    .getInstance()});
        } else {
    ...
    long tryGettingFiles(BinaryContent content, long start, boolean insert) {
        ...
        total = 0L;
        for (int i = files.length - 1; i >= 0; --i) { // for some reason they are given in reverse
                                                      // order
            File file = new File(files[i]);
            try {
