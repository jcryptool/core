...
/**
 * Status line component of the editor. Displays the current position and the insert/overwrite
 * status.
 */
public class StatusLine extends Composite {
    ...
    private void initialize(boolean withSeparator) {
        ...
        GridData gridData1 = new GridData(/* SWT.DEFAULT */(11 + 10 + 12 + 3 + 10 + 12)
        ...
        GridData gridData2 = new GridData(/* SWT.DEFAULT */(7 + 3 + 9 + 2 + 9 + 8 + 6)
                * fontMetrics.getAverageCharWidth(), SWT.DEFAULT);
        ...
        GridData gridData3 = new GridData(/* SWT.DEFAULT */(textOverwrite.length() + 2)
                * fontMetrics.getAverageCharWidth(), SWT.DEFAULT);
    ...
    public void updateInsertModeText(boolean insert) {
        if (isDisposed() || insertMode.isDisposed())
            return;
    ...
    public void updatePositionText(long pos) {
        if (isDisposed() || position.isDisposed())
            return;
    ...
    public void updateValueText(byte val) {
        ...
        String valText = "Value: " + val + " (dec) = " + Integer.toHexString(0x0ff & val)
                + " (hex) = " + valBinText.substring(valBinText.length() - 8) + " (bin)";
        // String valText = String.format("Value: %1$d (dec) = %1$X (hex) = %2$s (bin)", val,
        // valBinText.substring(valBinText.length()-8));
    ...
    public void updateSelectionText(long[] sel) {
        ...
        String selText = "Selection: " + sel[0] + " (0x" + Long.toHexString(sel[0]) + ") - "
                + sel[1] + " (0x" + Long.toHexString(sel[1]) + ")";
        // String selText = String.format("Selection: %1$d (0x%1$X) - %2$d (0x%2$X)", sel[0],
        // sel[1]);
