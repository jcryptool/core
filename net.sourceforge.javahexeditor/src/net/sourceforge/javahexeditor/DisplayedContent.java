...
/**
 * StyledTextContent customised for content that fills up to one page of the StyledText widget. No
...
public class DisplayedContent implements StyledTextContent {
    ...
    /**
     * Replaces part of the content with new text. Works only when the new text length is the same
     * as replaceLength (when the content's size won't change). For other cases use
     * <code>setText()</code> or <code>shiftLines()</code> instead.
     *
	...
	public void setText(String text) {
        ...
        for (Iterator i = myTextListeners.iterator(); i.hasNext();)
            ((TextChangeListener) i.next()).textSet(changedEvent);
    }
	...
	public void shiftLines(String text, boolean forward) {
        ...
        for (Iterator i = myTextListeners.iterator(); i.hasNext();)
            ((TextChangeListener) i.next()).textChanging(event);

        myData.insert(event.start, text);

        TextChangedEvent changedEvent = new TextChangedEvent(this);
        for (Iterator i = myTextListeners.iterator(); i.hasNext();)
            ((TextChangeListener) i.next()).textChanged(changedEvent);
