 ...
 * time difference lower than MERGE_TIME. Block actions are sequences of BinaryContent.Range. Single
...
public class ActionHistory {
    ...
    /**
     * Waiting time before a single action is considered separate from the previous one. Current
     * value is 1500 milliseconds.
     */
    ...
    private long actionExclusiveEnd() {
        ...
        if (myCurrentAction != null && myCurrentAction.size() > 0) {
            BinaryContent.Range highest = (BinaryContent.Range) myCurrentAction.get(myCurrentAction
                    .size() - 1);
    ...
    /**
     * Adds a list of deleted integers to the current action. If possible, merges integerList with
     * the list in the previous call to this method.
     *
     * @param position starting delete point
     * @param integerList deleted integers
     * @param isSingle used when integerList.size == 1 to tell whether it is a single or a piece of
     *        a block delete. When integerList.size() > 1 (a block delete for sure) isSingle is
     *        ignored.
     */
    ...
    void eventPreModify(Integer type, long position, boolean isSingle) {
        if (type != myCurrentActionType || !isSingle
                ...
                && actionPosition() != position && actionPosition() - 1L != position) {
