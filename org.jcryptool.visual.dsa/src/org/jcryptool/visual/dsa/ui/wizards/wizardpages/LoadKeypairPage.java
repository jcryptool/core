...
public class LoadKeypairPage extends WizardPage {
    ...
    private void initKeystoreConnection() {
        ...
        for (final Enumeration<String> aliases = ksm.getAliases(); aliases.hasMoreElements();) {
            ...
            if (alias.getClassName().equals(ElGamalPrivateKey.class.getName())) {
                keyStoreItems.put(
                        alias.getContactName() + " - " + alias.getKeyLength() + "Bit - " + alias.getClassName(), alias);
            }
        }
    }
    ...
    public void createControl(final Composite parent) {
        ...
        new Label(composite, SWT.NONE).setText("Bitte wählen Sie ein Schlüsselpaar aus der Liste aus.");
        ...
        l.setText("Bitte geben Sie das zu diesem Schlüsselpaar gehörige Passwort ein.\nSollte kein Passwort gesetzt sein, lassen Sie das Feld einfach leer.");
        l.setLayoutData(gd);
    ...
    private void checkComplete() {
        final boolean complete = privateAlias != null && !passfield.getText().equals("");
        if (complete) {
