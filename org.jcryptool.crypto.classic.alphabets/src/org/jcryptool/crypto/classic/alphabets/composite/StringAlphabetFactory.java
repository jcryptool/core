...
public class StringAlphabetFactory {
	...
	public static void main(String[] args) {
		AtomAlphabet a_z = new AtomAlphabet("abcdefghijklmnopqrstuvwxyz");
		AtomAlphabet A_Z = new AtomAlphabet("abcdefghijklmnopqrstuvwxyz".toUpperCase());
		AtomAlphabet digits = new AtomAlphabet("0123456789");
		AtomAlphabet random = new AtomAlphabet("asjhgjkghjghgkjKJJHKJ§/(/");
		
		System.out.println(a_z);
		System.out.println(A_Z);
		System.out.println(digits);
		System.out.println(random);
		System.out.println(StringAlphabetFactory.stringToList(random.getCharacterSet()));

		CompositeAlphabet printable = new CompositeAlphabet(a_z, A_Z, digits, random);
		
		System.out.println(buildStringRepresentation(printable));
		
		for(Character c: a_z.getCharacterSet()) {
			if(! printable.contains(c)) System.out.println("!!");
		}
		for(Character c: A_Z.getCharacterSet()) {
			if(! printable.contains(c)) System.out.println("!!");
		}
		for(Character c: digits.getCharacterSet()) {
			if(! printable.contains(c)) System.out.println("!!");
		}
