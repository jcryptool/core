...
/**
 * @author SLeischnig A standard bar, for use with the Graph class. Add a Bar to
 *         a Graph to display it. Designed for extension for customized use.
 */
public class Bar {
	...
	// graphical context
	protected GC gc;
	// order of painting (lower numbers are drawn underlaying) and x-axis position
	// (index)
	protected int order, index;

	/**
	 * Creates a default bar which is capable of being drawn on any graphical
	 * context. <br>
	 * You cann add it to a Graph class/subclass. This bar is black and has a fixed
	 * 65% width and the drawing when leaving it unchanged.
	 * 
	 * @param pHeight
	 * @param pIndex  The x-Axis position
	 * @param pOrder  lower values: drawn in the background. higher values: drawin
	 *                in the foreground.
	 */
	public Bar(double pHeight, int pIndex, int pOrder) {
		setHeight(pHeight);
		setWidth(-1); // => standard values in Graph class apply
		// setBox(new Rectangle(0, 0, 0, 0)); => null => standard values in Graph class
		// apply
		// setColorMainBar(new MColor("000000", 255)); => null => standard values in
		// Graph class apply
		gc = null;
		index = pIndex;
		order = pOrder;
	...
	/**
	 * Creates a default bar which is capable to be drawn on any graphical context.
	 * <br>
	 * You cann add it to a Graph class/subclass. <br />
	 * You may have to deliver the gc and drawing outline box, if you're not using
	 * the default Graph Class
	 * 
	 * @param pHeight       Height of the bar as a ratio of the draw box height
	 *                      (min: 0, max: 1)
	 * @param pWidth        Width as a ratio of the draw box width #
	 * @param pIndex        The x-Axis position
	 * @param pOrder        lower values: drawn in the background. higher values:
	 *                      drawin in the foreground.
	...
	public void setColorMainBar(MColor colorMainBar) {
		this.colorMainBar = colorMainBar;
	}

	public MColor getColorMainBar() {
		return colorMainBar;
	}
	...
	/**
	 * Sublasses may add additional content to the upper and lower description
	 * area<br />
	...
	/**
	 * draws the bar on a graphical context. This is a funbction call without
	 * parameters whch uses the fields in the Bar class.
	 */
	public void drawBar(String topLabel) {
		drawBar(getBox(), getHeight(), getWidth(), getColorMainBar(), topLabel, gc);
	}
	 ...
	 * 
	 * @param myBox          the drawing area (and limitation for a 100% height and
	 *                       width bar)
	...
	public final void drawBar(Rectangle myBox, double myHeight, double myWidth, MColor myColorMainbar, String topLabel,
			GC myGC) {
