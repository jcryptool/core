...
/**
 * @author SLeischnig a Bar derivate that displays labels and uses a overlay-ish
...
public class OverlayLabelBar extends LabelBar {
	...
	/**
	 * Creates a default label overlay bar which is capable of being drawn on any
	 * graphical context. <br>
	...
	/**
	 * Creates a default label overlay bar which is capable to be drawn on any
	 * graphical context. <br>
	 ...
	 *                      x-axis-label)
	 */
	public OverlayLabelBar(double height, double width, int index, int order, MColor colorMainBar, String pUpperLabel,
			String pLowerLabel) {
	 ...
	 *                      x-axis-label)
	 */
	public OverlayLabelBar(double height, double width, int index, int order, Rectangle box, MColor colorMainBar, GC gc,
	 ...
	 * 
	 * @param myBox the drawing area (and limitation for a 100% height and width
	 * bar)
	 * 
	...
	public void drawBar(Rectangle myBox, double myHeight, double myWidth, MColor myColorMainbar, GC myGC) {
		...
		for (int k = myBar.y + myBar.height; k > myBar.y; k -= transparencySteps) {
			double actNormDistrPercentage = (double) (myBar.y + myBar.height - k) / (double) (myBar.height);
			...
			int alpha = (int) Math.round((getTransparencyHighest() - getTransparencyLowest())
					* (actNormDistrPercentage + nextNormDistrPercentage) / 2) + getTransparencyLowest();
