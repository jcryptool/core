...
/**
 * Java port of the reference implementation of Keccack-1600 permuation from
...
public final class Keccack1600 {
    ...
    public void permute() {
        /*
         * for (int i=firstRound; i < NR_ROUNDS; ++i) { theta(); rho(); pi(); chi(); iota(i); }
         */
        long out0 = state[0];
        long out1 = state[1];
        long out2 = state[2];
    ...
    final static long rol64(long l, int offset) {
        return (offset != 0) ? ((l << offset) | (l >>> (64 - offset))) : l;
    }
    ...
    static void writeRoundToStdout() {
        ...
        for (int x = 0; x < 5; ++x) {
            for (int y = 0; y < 5; ++y) {
                ...
                if (rotationOffset == 0) {
                    ...
                    System.out.println("long piOut" + outPiIndex + " = ((" + theta + "  << " + rotationOffset + ") | ("
                            + theta + " >>> " + rotationRight + " ));");
                }
            }
        }
        ...
        for (int y = 0; y < 5; y++) {
            for (int x = 0; x < 5; x++) {
                ...
                System.out.println("out" + piOutIndex1 + " = piOut" + piOutIndex1 + " ^ ((~piOut" + piOutIndex2
                        + ") & piOut" + piOutIndex3 + ");");
            }
        }
        /* iota */
