...
/**
 * A cryptographic random generator based on Keccack-1600 and the duplex construction. Supports
 * re-seeding and forward secrecy through forgetting past state.
 * 
 * Note that this generator must be synchronized externally if used concurrently from multiple
 * threads.
 *
 */
public final class DuplexRandom {
    ...
    /**
     * @param capacityInBits. This should be approximately 2x the desired security level. For
     *            efficiency it should be chosen such that we get a multiple of 64-bytes output per
     *            permutation. This generator used 3 bits of padding so pick capcitiyInBits = n*64 -
     *            3 with n between [4, 25>. Higher n gives higher security, but lower performance.
     */
    ...
    /**
     * (Re)seed with the supplied seed. If you seed with a static value before the first call to
     * getBytes() you will get a reproducible random sequence with a security that depends upon the
     * security of the seed. Seeds are accumulated and not used before you have supplied at least 16
     * bytes/128 bits.
     * 
     * @param seed
     * @param off
