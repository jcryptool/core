...
public class DividerGameUtil {
	...
	public static void dumpNotifications(GameMachineEvent event) {
		...
		System.out.println("Chosen Number: " + state.getChosenNumber());
		System.out.println("Player current: " + state.getPlayerCurrentRound().getName());
		if (state.getPlayerLastRound() != null) {
			System.out.println("Player last: " + state.getPlayerLastRound().getName());
		} else {
			System.out.println("Player last: NULL");
		}
		System.out.println("List of numbers: " + listOfNumbers);
		if (state.getEliminatedNumbers() != null) {
			System.out.println("Eliminated numbers: " + eliminatedNumbers);
		} else {
