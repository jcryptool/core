...
public class NgramView extends ViewPart
{
	...
	public void createPartControl(Composite parent)
	{
		...
		cb_LoadText.setItems(new String[] {Messages.NgramView_6, Messages.NgramView_7, Messages.NgramView_8, Messages.NgramView_9});
        ...
        //#be cb_ChooseLang.setItems(new String[] {"English", "German"}); //$NON-NLS-1$ //$NON-NLS-2$
		...
		txt_CipherText.addModifyListener(new ModifyListener()
		{
			...
			{
				txt_ResultText.setText(""); //$NON-NLS-1$
				grp_CipherText.setText(Messages.NgramView_29 + Messages.NgramView_30 + txt_CipherText.getText().length() +Messages.NgramView_0); //$NON-NLS-3$ //$NON-NLS-1$
			}
		});		
		     ...
		     public void mouseDown(MouseEvent e)
	            {
	                try
	                {
	                    ...
	                    {
	                        lbl_FileInfo.setText(Messages.NgramView_37 + (path.length() - 60 < 0? "": "...")   //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
	                        								   + path.substring(Math.max(path.length() - 60, 0), Math.max(path.length(), 60)) 
	                        								   + Messages.NgramView_40 +fis.available()+Messages.NgramView_1); //$NON-NLS-2$ //$NON-NLS-1$
	                    }
	                    fis.read(content);
	                    fis.close();
	                }
