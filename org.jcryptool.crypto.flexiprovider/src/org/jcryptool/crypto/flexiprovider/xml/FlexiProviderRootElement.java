...
//			// parameter specs
	...
	private static void initParameterSpecs(Element parameterSpecsParent) {
		...
		while (it.hasNext()) {
			...
			List<Element> constructorChildren = new ArrayList<Element>(currentSpec.getChildren(AlgorithmsXMLConstants._Constructor));
			...
			while (consIt.hasNext()) {
				...
				List<Element> parameterChildren = new ArrayList<Element>(currentConstructor.getChildren(AlgorithmsXMLConstants._Parameter));
	...
	protected FlexiProviderRootElement(Element element) throws InvalidAlgorithmsXMLElementException {
		if (element.getName().equals(AlgorithmsXMLConstants._FlexiProvider)) {
			...
			// KeyGenerators
			initKeyGenerators(RegistryType.SECRET_KEY_GENERATOR, element.getChild(AlgorithmsXMLConstants._SecretKeyGenerators));
			initKeyGenerators(RegistryType.KEY_PAIR_GENERATOR, element.getChild(AlgorithmsXMLConstants._KeyPairGenerators));
			...
			initKeyPairAlgorithms(RegistryType.ASYMMETRIC_BLOCK_CIPHER, element.getChild(AlgorithmsXMLConstants._AsymmetricBlockCiphers));
			initKeyPairAlgorithms(RegistryType.ASYMMETRIC_HYBRID_CIPHER, element.getChild(AlgorithmsXMLConstants._AsymmetricHybridCiphers));
