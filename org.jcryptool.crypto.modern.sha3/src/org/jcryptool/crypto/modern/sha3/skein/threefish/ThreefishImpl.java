...
public class ThreefishImpl {
    ...
    private static void logBlock(final Level logLevel, final String identifier, final String counterType,
            final int counter, final long[] block) {
        StringBuilder sb = new StringBuilder();
        if (counterType == null) {
            sb.append(String.format(" --- %s (%d) --- %n", identifier, counter));
        } else {
     ...
     */
    private static final int[][] R4 = { {14, 16}, {52, 57}, {23, 40}, {5, 37}, {25, 33}, {46, 12}, {58, 22}, {32, 32}};

    /**
     * Rotational constants Rd,j for Nw = 8.
     */
    private static final int[][] R8 = { {46, 36, 19, 37}, {33, 27, 14, 42}, {17, 49, 36, 39}, {44, 9, 54, 56},
            {39, 30, 34, 24}, {13, 50, 10, 17}, {25, 29, 39, 43}, {8, 35, 56, 22}};

    /**
     * Rotation constants Rd,j for Nw = 16.
     */
    private static final int[][] R16 = { {24, 13, 8, 47, 8, 17, 22, 37}, {38, 19, 10, 55, 49, 18, 23, 52},
    ...
    public void init(final long[] key, final long[] tweak) {
        logger.log(Level.FINER, String.format(" === Starting Threefish (blocksize = %d, rounds = %d) === %n",
                blockSize, nr));
