...
/**
 * Provides basic labels for adaptable objects that have the <code>IWorkbenchAdapter</code> adapter
 * associated with them. All dispensed images are cached until the label provider is explicitly
...
public class FileExplorerLabelProvider extends LabelProvider implements IColorProvider,
    ...
    /**
     * Listener that tracks changes to the editor registry and does a full update when it changes,
     * since many workbench adapters derive their icon from the file associations in the registry.
     */
    ...
    /**
     * Returns an image descriptor that is based on the given descriptor, but decorated with
     ...
     * @return IWorkbenchAdapter or<code>null</code> if the adapter is not defined or the object is
     *         not adaptable.
     */
     ...
     *
     * @param o the object to look up.
     * @return IWorkbenchAdapter2 or<code>null</code> if the adapter is not defined or the object is
     *         not adaptable.
     */
    ...
    public final String getText(Object element) {
        ...
        /*
         * changed to display drive labels - start
         * @auhtor mwalthart
         */
        return decorateText(label.equals("") ? ((IFileStore) element).toString() : label, element); //$NON-NLS-1$
