...
/**
 * Pastes the clipboard content (file(s) or directory/ directories) into the selected folder.
 * To distinguish between a paste operation after copy or a cut, the cut operation inserts an
 * additional TextTransfer instance into the clipboard with the content <b>CutOperation</b>.
 * This is the indicator for the paste operation to delete the source file after creating the
 * target file. In case of a normal copy operation this TextTransfer instance does not exist,
...
public class PasteHandler extends AbstractHandler {
            ...
            if (data != null && data.length > 0) {
                for (int i = 0, length = data.length; i < length; i++) {
                    ...
                    try {
                        ...
                        LogUtil.logError(FileExplorerPlugin.PLUGIN_ID, "Error while pasting clipboard content", ex, false); //$NON-NLS-1$
                    } finally {
    ...
    private String enterFilenameDialog(String filename, String directory) {
        InputDialog dialog = new InputDialog(null, Messages.PasteHandler_0,
                Messages.PasteHandler_1, filename, null);
