...
public class WOTSPlus implements OTS {
    ...
    /**
     * 
     * @param input n-byte string to calculate the hash of
     * @param i start index
     * @param s number of steps
     * @param seed the seed for the bitmask and key generation
     * @param otsAdrs the address construct for the bitmask and key generation
     * @return the s times hashed input
     */
    ...
    public byte[][] pkFromSig(String sig, byte[] message, byte[] seed, OTSHashAddress otsAdrs) {
        ...
        for (int i = 0; i < l; i++) {
            otsAdrs.setChainAddress(i);
            pk[i] = chain(signature[i], messageW[i], w - 1 - messageW[i], seed, otsAdrs); // BIgInteger
                                                                                          // is used
                                                                                          // to
                                                                                          // convert
                                                                                          // byte[]
