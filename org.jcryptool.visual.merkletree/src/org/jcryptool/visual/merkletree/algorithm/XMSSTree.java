...
public class XMSSTree implements ISimpleMerkle {
    ...
    public byte[] generateLTree(byte[][] pKey, byte[] seed, LTreeAddress adrs) {
        ...
        while (len > 1) {
            ...
            // zuck: Anpassen der Anzahl an Nodes bzw. setzen der Anzahl der Nodes auf der neuen
            // Höhe
    ...
    /**
     * Initialises the tree. Requires seed, bitmaskSeed and leafCounter to be set beforehand and
     * then the execution of generateKeyPairsAndLeaves()
     */
    @Override
    ...
    @Override
    /**
     * Selects the Signature Algorithm defautl Algorithm is the WOTSPlus Algorithm after the
     * Algorithm is selected the method -> generateKeyPairsAndLeaves() is called
     */
    ...
    @Override
    /**
     * signs the given Message String using the the defined OTS Algorithm returns the Signature
     */
        ...
        for (int k = 0; k < getTreeHeight(); k++) {
            ...
            if (Math.floor((double) index / (1 << k)) % 2 == 0) {
                ...
                node[1] = new XMSSNode(rand_hash(node[0].getContent(), Converter._hexStringToByte(splitted[3 + k]),
                        bitmaskSeed, lAdrs)); // splitted[3] is auth[0] in signature
            } else {
    ...
    /**
     * Defines the node as leaf if it is and set the index of the node. Also adds the node into the
     * treeArray list and sets the binary presentation of the authentication path
     * 
