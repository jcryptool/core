...
/**
 * In this class, an Integer is polled from a finite number of Integers.
 * See {@link PolledValue} for more information. 
 * 
 * @author Simon L
 *
 */
public class PolledPositiveInteger extends PolledValue<Integer> {
	...
	public void addChoice(Integer choice, double initialPossibility) {
		...
		else throw new IllegalArgumentException("only positive Integers allowed to be added.");
	}
	
	public boolean containsAllValuesInRange(int start, int end) {
		if(start > end) throw new IllegalArgumentException("start must be less than or equal to end");
