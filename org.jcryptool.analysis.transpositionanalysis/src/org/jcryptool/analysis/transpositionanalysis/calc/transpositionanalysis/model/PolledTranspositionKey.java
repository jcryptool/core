...
public class PolledTranspositionKey {
	...
	private static final PolledPositiveInteger NOVALUE(int positionCount) {
		...
		for(int i=0; i<positionCount; i++) result.addChoice(i);
		return result;
	}
	...
	/**
	 * Creates a PolledTranspositionKey from an array. Format: see {@link #fromArray(double[][])}
	 *
	 ...
	 * position b will be placed in position a in the ciphertext. Specifying a high possibility
	 * for a value at a given position means, that it is very likely that the polled key
	 * will have this value on the said position in the end.
	 * <br /> <br />
	 ...
	 * @param possibility the possibility vot the value to be actually in the given position (use constants from {@link PolledValue} for example).
	 */
	...
	/**
	 * Combines the possibility for a value at the given position of the key with a given one. <br />
	 ...
	 * @param position the position in the key
	 * @param value the value that has to stand at this position.
	 * @param possibility the possibility the existing possibility will be combined with (you can use constants from {@link PolledValue} for example).
	 */
	 ...
	 * are bigger than this new length, will be removed. In the end, there will be n polls for key positions,
	 * when n is the new length, and the method {@link #toArray()} will return a n x n array.
	 *
	...
	public void setLength(int length) {
		...
		for(int i=0; i<length; i++) {
			...
			//remove all entries from a poll for a position, that are greater than the length
			removelist.clear();
			...
			// make sure, that in all Polls for positions are only the values between 0 and length
	...
	private int[] combineRecursiveChoices(int thisDepthChoice, int[] nextDepthChoice) {
		...
		for(int i=1; i<nextDepthChoice.length+1; i++) {
			result[i] = nextDepthChoice[i-1] >= thisDepthChoice ?
					nextDepthChoice[i-1] + 1 :
					nextDepthChoice[i-1];
		}
	...
	private double[][] makeSubPollArray(int choiceIndexOfRowZero, double[][] pollArray) {
		if(! (pollArray.length > 1 && pollArray[0].length > 1))throw new IllegalArgumentException("Can't crate sub - Poll-array from 1-element-array.");
		if(choiceIndexOfRowZero >= pollArray.length) throw new IllegalArgumentException("choiceIndex is greater than poll array dimensions. cannot create sub - poll-array.");
	...
	private int[] getBestChoice(double[][] pollArray) {
		...
		if(pollArray.length == 1) return new int[]{0}; // take the remaining piece
		// not-trivial cases
		int[] bestChoice = null;
		double bestChoiceValue = Double.MIN_VALUE;
		for(int i=0; i<pollArray.length; i++) {
			double[][] subArray = makeSubPollArray(i, pollArray);
			//heuristic cost is the biggest cost that could possibly reached with choice i (estimated)
	...
	private double calcHeuristicCostForChoiceAndFollowingSubarray(double choiceValue,
			double[][] subArray) {
	...
	/**
	 * Calculates the best TranspositionKey that would match the given possibility data.
	 *
	 * @return a TranspositionKey which is the best match for this PolledTranspositionKey.
	 */
	public TranspositionKey getBestChoice() {
		double[][] array = toArray();
		if(! (array.length > 0 && array[0].length > 0)) throw new IllegalArgumentException("polled key has no content - cannot calculate best choice");
	...
	private double[] calcBestValuePerPosList(double[][] array) {
		...
		for(int i=0; i<valueListAsArray.length; i++) valueListAsArray[i] = maxValues.get(i);
	 ...
	 * relatively high one*. Such, a polled key of length n is always represented by
	 * a double array with dimensions n x n.<br /><br />
	 *
	 * * because a possibility of 1 means 50%, and 6 is normally a value that specifies "Very high possibility". see possibility constants in {@link PolledValue}, e. g. {@link PolledValue#POSSIBILITY_VERY_LIKELY}
	 *
	 * @param arrayRepresentation a representation of the polled key that shall be
	 * loaded, as described above (length x length - array)
	 */
