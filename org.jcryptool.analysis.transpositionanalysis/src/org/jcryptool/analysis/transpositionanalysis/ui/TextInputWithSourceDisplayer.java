...
public class TextInputWithSourceDisplayer extends Composite {
	 ...
	 *            the parent
	 * @param layoutRoot
	 *            the composite which is used to layout when hiding / showing
	 *            controls. must be "big" enough, that it itself won't need to
	 *            resize.
	 * @param displayStyle
	 *            the display style (see {@link Style})
	...
	private void refreshDisplay() {
		...
		if (displayStyle.images) {
			...
			if (originComposite != null) {
				...
				if (displayOrigin) manualOriginDisplayer.setText(text.userInputOrigin);
	...
	private void displayOrigin(boolean b, boolean layout) {
		...
		if (lDataOriginDisplayer.exclude == b) { // if sth needs to change
			...
			if (layout) this.layoutRoot.layout(new Control[] { originComposite });
		}
	}
	...
	/**
	 * Tries to open the given IEditorInput in the Editor associated with the
	 * given ID. This method must be executed in an async way since a Job may be
	 * used to execute the cryptographic operation.
	 * 
	 * @param input
	 *            The IEditorInput that shall be displayed
	 * @param editorId
	 *            The ID of the Editor that is supposed to open
	 */
