...
public class KeyPairNode extends TreeNode {
	...
	public KeyPairNode(KeyStoreAlias privateAlias, KeyStoreAlias publicAlias) {
		super(getOperation(privateAlias, publicAlias) +  Messages.getString("Label.KeyPair") + " ("+ Messages.getString("Label.KeyStrength") +": "  //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					+ ((getKeyLength(privateAlias, publicAlias) > 0) ? getKeyLength(privateAlias, publicAlias) : "n/a") + ")"/* + (addCounter==0?"":" ("+addCounter+")")*/); //$NON-NLS-1$ //$NON-NLS-2$
		
		this.privateAlias = privateAlias;
		this.publicAlias = publicAlias;
		
//		super(getOperation(privateAlias, publicAlias) + " " + KeyStorePlugin.getResourceBundle().getString("Label.KeyPair") + " ("+ KeyStorePlugin.getResourceBundle().getString("Label.KeyStrength") +": " + getKeyLength(privateAlias, publicAlias) + ")");
	...
	public String getName() {
		return getOperation(privateAlias, publicAlias) +  Messages.getString("Label.KeyPair") + " ("+ Messages.getString("Label.KeyStrength") +": "  //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	...
	private static String getOperation(KeyStoreAlias privateAlias, KeyStoreAlias publicAlias) {
		...
		} else return "<undefined>"; //$NON-NLS-1$
	}
