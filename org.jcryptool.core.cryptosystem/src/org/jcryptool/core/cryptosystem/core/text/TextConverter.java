...
public abstract class TextConverter<C> {
	public final static TextConverter<Character> CHAR_TEXT_CONVERTER = new TextConverter<Character>() {
		@Override
		public List<Character> parseString(String s,
	 ...
	 * when encountering content that does not belong to the given alphabet or utterly
	 * unexpected content.
	 * 
	 * @param s the string to parse
	 * @param alphabet the alphabet which provides the possible list elements
	 * @return a list of alphabetic elements
	 * @throws ElementNotInAlphabetException when unexpected content occurs (e. g. elements which are not
	 *             parseable within the alphabet)
	 */
	public abstract List<C> parseString(String s, Alphabet<C> alphabet)
			throws ElementNotInAlphabetException;
}