...
/**
 * Models a cryptosystem on lists, which can parse strings by 
 * using TextCompatibleAlphabet objects as alphabets.
 * 
 * @see Cryptosystem
 * 
 * @author Simon L
 */
public abstract class TextCompatibleCryptosystem<P, C, K extends Key> extends
		Cryptosystem<P, C, K> {
	 ...
	 */
	public TextCompatibleCryptosystem(
			TextCompatibleAlphabet<P> plainTextAlphabet,
	...
	/**
	 * Encrypts a string, by parsing it with the TextCompatibleAlphabet given
	 * at object creation.
	 * 
	 * @throws ElementNotInAlphabetException when unexpected contents (e. g. non-alphabet-elements/not compliant content to the parse method in the {@link TextCompatibleAlphabet}) were encountered at String parsing.
	 * @see #encrypt(List, Key)
	 * @see TextCompatibleAlphabet#parseString(String)
	 */
	public String encryptTextToText(String plainText, K key)
			throws ElementNotInAlphabetException {
		List<P> plainList = textCompatiblePlaintextAlphabet
				.parseString(plainText);
		return listToString(encrypt(plainList, key));
	}

	/**
	 * Decrypts a string, by parsing it with the TextCompatibleAlphabet given
	 * at object creation.
	 * 
	 * @throws ElementNotInAlphabetException when unexpected contents (e. g. non-alphabet-elements/not compliant content to the parse method in the {@link TextCompatibleAlphabet}) were encountered at String parsing.
	 ...
	 * @see TextCompatibleAlphabet#parseString(String)
	 */
	public String decryptTextToText(String ciphertext, K key)
			throws ElementNotInAlphabetException {
		List<C> cipherList = textCompatibleCiphertextAlphabet
				.parseString(ciphertext);
		return listToString(decrypt(cipherList, key));
	}

	/**
	 * Converts a list of object into a String by concatenating the
