 ...
 * 
 * @param <A>
 *            the minimum class for the plain- and ciphertext alphabets.
 * @author Simon L
 */
public class VigenereCryptosystem<A> extends Cryptosystem<A, A, VigenereKey<A>> {
	...
	public List<A> encrypt(List<? extends A> plainText, VigenereKey<A> key)
		...
		for (A plainTextElement : plainText) {
			if (!getPlainTextAlphabet().getContent().contains(
					plainTextElement)) {
				throw new ElementNotInAlphabetException();
			}
			chiffre.add(rotateElementAgainstAlphabet(plainTextElement,
					intKey.get(counter % intKey.size()),
	...
	@Override
	public List<A> decrypt(List<? extends A> cipherText, VigenereKey<A> key)
			throws ElementNotInAlphabetException {
		VigenereKey<A> invertedKey = invertKey(key, this.getPlainTextAlphabet());
		return encrypt(cipherText, invertedKey);
	}

	public static <A> VigenereKey<A> invertKey(VigenereKey<A> key,
	...
	public static <A> A rotateElementAgainstAlphabet(A plainTextElement,
			int rotAmount, Alphabet<? extends A> basicAlphabet) {
		return basicAlphabet.getContent()
				.get((basicAlphabet.getContent().indexOf(
						plainTextElement) + rotAmount)
						% basicAlphabet.getContent().size());
	}
