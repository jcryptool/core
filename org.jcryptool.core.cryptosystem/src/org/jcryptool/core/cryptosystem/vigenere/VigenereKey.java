...
public class VigenereKey<A> extends Key {
	 ...
	 * @throws IllegalArgumentException if the key is empty or if one element of the key list is not member of the alphabet
	 */
	public VigenereKey(List<? extends A> key, Alphabet<A> alphabet) {
		String checkResult = checkKey(key, alphabet);
		if(! (checkResult==KEY_VERIFICATION_RESULT_OK)) {
	...
	public VigenereKey(int[] key, Alphabet<? extends A> alphabet) {
		...
		for(int keyElem: key) intKey.add(keyElem);
	...
	private static <A> List<A> calculateAlphabetElementKey(int[] key,
			Alphabet<A> alphabet) {
	...
	public static String checkIndexbasedKey(int[] key,
			Alphabet<?> alphabet) {
		if(key.length != 0) {
			for(int keyElement: key) {
	...
	private static String elementOfIndexbasedKeyOutOfBoundsVerificationResult(
			final int keyElement) {
