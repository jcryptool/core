 ...
 *******************************************************************************/
// -----END DISCLAIMER-----
package org.jcryptool.analysis.fleissner2.key;
...
public class Grille {
    ...
    public int[] translateKeyToLogic() {
        
        int holes, i=0;
        if (key.getSize()%2==0) {
            holes = (int) Math.pow(key.getSize(), 2)/4; 
        }
        else {
            holes = (int) (Math.pow(key.getSize(), 2)-1)/4;
        }
        int[] newKey = new int[holes*2];
