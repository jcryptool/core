...
public class CrtVerViewController {
	...
	/**
	 * Method to get month from a calendar dependent on the selection of a scale
	 * and it's default selection.
	 * 
	 * @param selection
	 *            The actual selection getting with .getSelection()
	 * @param default_selection
	 *            The default Selection of Scale Receiver, it will be subtracted
	 *            from the value
	 * @param format
	 *            The date format to represent
	...
	public String scaleUpdate(int selection, int default_selection,
		...
		if (default_selection == 360) {
			if (((selection - default_selection) % 2) == 0) {
				...
				calendar.add(Calendar.MONTH,
						((selection + 1) - default_selection) / 2);
			}
		} else {
	...
	/**
	 * Validates the three Models with the help of Class ShellModelVerifier and
	 * CertPathVerifier.
	 * 
	 * @param mode
	 *            1=Shell Model, 2=Modified Shell Model, 3=Chain Model
	 */
	public void validate(int mode) {
		...
		try {
			...
			if (flag) {
				cpv = new CertPathVerifier(getRootCA(), getCA(), getTN(),
						verificationDate, signatureDate);
				...
				ArrayList<String> errors = cpv.verifyChangedDate(mode,
						fromCert, thruCert, fromCa, thruCa, fromRootCa,
	...
	/**
	 * Updating local variables with the values of the Textfield and Labels from
	 * Composite Class.
	 */
	...
	public void reset() {
		...
		composite.btnLoadCa.setForeground(SWTResourceManager
				.getColor(SWT.COLOR_BLACK));
	...
	public void loadCertificate(ChooseCertPage p, X509Certificate cert,
		...
		case 1: // [1] UserCert
			...
			setLogText("\"" + contact_name + "\" "
					+ Messages.crtVerification_status_UserCertLoaded);
			break;
		case 2: // [2] Cert
			setCA(cert);
			setScales(2);
			...
			composite.btnLoadCa.setForeground(SWTResourceManager
					.getColor(SWT.COLOR_DARK_GREEN));
			setLogText("\"" + contact_name + "\" "
					+ Messages.crtVerification_status_CaLoaded);
			break;
		case 3: // [3] RootCert
			setRootCA(cert);
			setScales(3);
			...
			composite.btnLoadRootCa.setForeground(SWTResourceManager
					.getColor(SWT.COLOR_DARK_GREEN));
			setLogText("\"" + contact_name + "\" "
					+ Messages.crtVerification_status_RootCaLoaded);
			break;
		}
	...
	public void updateElements(Label l, Scale s, int default_selection) {
		l.setText(scaleUpdate(s.getSelection(), default_selection,
				getDateformat1()));
		s.setToolTipText(scaleUpdate(s.getSelection(), default_selection,
				getDateformat2()));
	 ...
	 * 
	 * @param s
	 *            The String to append to Log Field
	 */
	...
	/**
	 * Prints the NotBefore, NotAfter of all three certificates and Verification
	 * and Signation Time to the log
	 */
	public void logValidityDates() {

		parseDatesFromComposite();

		setLogText(Messages.CrtVerViewComposite_RootCa + ": "
				+ Messages.CrtVerViewComposite_validFrom + " "
				+ getFromRootCa() + ", "
				+ Messages.CrtVerViewComposite_validThru + " "
				+ getThruRootCa());
		setLogText(Messages.CrtVerViewComposite_Ca + ": "
				+ Messages.CrtVerViewComposite_validFrom + " " + getFromCA()
				+ ", " + Messages.CrtVerViewComposite_validThru + " "
				+ getThruCA());
