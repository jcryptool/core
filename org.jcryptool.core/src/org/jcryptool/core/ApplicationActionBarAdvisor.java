...
public class ApplicationActionBarAdvisor extends ActionBarAdvisor {
    ...
    private static CommandOrAction[] algorithmActions;
    ...
    private IContributionItem createContributionItem(IServiceLocator locator, Object contribSource, String commandId) {
    ...
    public /*static*/ IMenuManager createAlgorithmMenu() {
        ...
        // Sorted submenus map
        SortedMap<String, SortedMap<String, CommandOrAction>> sortedSubMenus
        	= new TreeMap<String, SortedMap<String, CommandOrAction>>(idComparator);

        // fill sub menu sorting map
        for (CommandOrAction algorithmAction : algorithmActions) {
            ...
            if (!sortedSubMenus.containsKey(type)) {
                ...
                // make a new submenu
                sortedSubMenus.put(type, new TreeMap<String, CommandOrAction>(menuStringsComparator));
            }

            if(algorithmAction.getHandler() != null) {
            	sortedSubMenus.get(type).put(((ShadowAlgorithmHandler)algorithmAction.getHandler()).getText(), algorithmAction);
            } else if(algorithmAction.getAction() != null) {
            	sortedSubMenus.get(type).put(algorithmAction.getAction().getText(), algorithmAction);
            } 
        }
        ...
        for (String subMenuKey : sortedSubMenus.keySet()) {
            ...
            Map<String, CommandOrAction> subMenuActions = sortedSubMenus.get(subMenuKey);
            for (String subMenuActionKey : subMenuActions.keySet()) {
            	CommandOrAction cmdOrAction = subMenuActions.get(subMenuActionKey);
            	if(cmdOrAction.getHandler() != null) {
        	        Command cmd = commandService.getCommand(cmdOrAction.getCommandId());
        	        cmd.define(((ShadowAlgorithmHandler)cmdOrAction.getHandler()).getText(), null, commandService.getCategory(CommandManager.AUTOGENERATED_CATEGORY_ID));
        	        cmd.setHandler(cmdOrAction.getHandler());
        	        final String commandId = cmdOrAction.getCommandId();
        	        
        	        AbstractContributionFactory contribFactory = new AbstractContributionFactory ("menu:" + subMenuKey, null) {
        	        	@Override
        	        	public void createContributionItems(IServiceLocator locator, IContributionRoot additions) {
        	        		...
        	        		additions.addContributionItem(createContributionItem(PlatformUI.getWorkbench(), null, commandId), null);
        	        	}
        	        };
        	        menuService.addContributionFactory(contribFactory);
            	} else if(cmdOrAction.getAction() != null) {
            		subMenu.add(cmdOrAction.getAction());
            	}
            }
        }
    ...
    private void registerAlgorithmActions() {
        ...
        for (CommandOrAction algorithmAction : algorithmActions) {
        	if(algorithmAction.getAction() != null)
        		register(algorithmAction.getAction());
        }
    }
