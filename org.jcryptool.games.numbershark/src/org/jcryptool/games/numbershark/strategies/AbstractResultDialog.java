...
public abstract class AbstractResultDialog extends TitleAreaDialog {
	...
	protected void createButtonsForButtonBar(Composite parent) {
		playSelected = createButton(parent, PLAY, Messages.ShowOptStrategy_9,
				true);
		playSelected.setEnabled(false);
		playSelected.addSelectionListener(playSelection);

		Button saveAsButton = createButton(parent, SAVE,
				Messages.ShowOptStrategy_8, true);
		saveAsButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				saveDialog.setFileName("log_numberShark_" + min + "-"
						+ max + ".csv");
		...
		sequences.addListener(SWT.MouseDown, new Listener() {
			...
			public void handleEvent(Event event) {
				if (event.button == 3) {
					...
					Point point = new Point(zeroPointTable.x + zeroPointShell.x
							+ event.x + 5, 104 + zeroPointShell.y + event.y);
					...
					LocalResourceManager resources = new LocalResourceManager(
							JFaceResources.getResources(), getShell());
					Image image = resources.createImage(NumberSharkPlugin
							.getImageDescriptor("icons/play_icon.png")); //$NON-NLS-1$
					play.setImage(image);
