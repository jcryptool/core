...
public class NumberSharkView extends ViewPart {
    ...
    public void addMoveToTable(int takenNumber, int[] lostNumbers) {
        ...
        isPrime = lostNumbers[0] == 0 ? false : NumberService.isPrime(lostNumbers[0]);
        ...
        for (int k = 1; k < lostNumbers.length; k++) {
            isPrime = lostNumbers[k] == 0 ? false : NumberService.isPrime(lostNumbers[k]);
     ...
     * 
     * @param numberOfFields the number of numbers you will play with
     * @param parent the parent Group where the content will be created in
     */
    public void createPlayingField(int numberOfFields) {
        ...
        if (hint != 0) {
            commandStateChanger.chageCommandState(CommandState.Variable.HINT_STATE, CommandState.State.HINT_ENABLED);
        } else {
            commandStateChanger.chageCommandState(CommandState.Variable.HINT_STATE, CommandState.State.HINT_DISABLED);
        }

    }

    /**
     * initializes a tab of the TabFolder with the playing buttons on the right side
     * 
     * @param selectedTab which tab is selecteed right now
     */
    public void initTab(int selectedTab) {
        ...
        for (int i = selectedTab * MAX_NUM_PER_TAB + 1; i <= (selectedTab + 1) * MAX_NUM_PER_TAB; i++) {
     ...
     * 
     * @param i number for which the ToolTip shall be created for
     ...
     * 
     * @param numToDeactivate deactivate that number in the game
     */
    public void deactivateNumber(int numToDeactivate) {
        ArrayList<Integer> lostNumbers = new ArrayList<Integer>();
        ArrayList<Integer> divisors = numberField[numToDeactivate - 1].getDivisors();
