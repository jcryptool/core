...
/**
 * Class for the Composite of Tabpage "Sphincs Description View" Depicts a Graph
 * of the Sphincs-Hypertree and a description
 * 
 * @author Philipp Guggenberger
 * 
 *         TODO This class has some conceptional and visual problems. According
 *         to H�lsing the tree shown is the top XMSS tree of SPINCS. This is not
...
public class SphincsTreeView extends Composite {
	...
	public SphincsTreeView(Composite parent, int style, ViewPart masterView, aSPHINCS256 sphincs, Signature signature) {
		...
		// Base item for collapseable part
		...
		// the composite which contains the expanded part
		...
		viewer.getControl().addPaintListener(new PaintListener() {
			...
			public void paintControl(PaintEvent e) {
				...
				double x = currentShellSize.x * 2.1; // x positions of Graph in treeComposite
				double y = currentShellSize.y * 3.0; // y positions of Graph in treeComposite
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (e.item instanceof GraphNode) {
					...
					if (n.isLeaf()) {
						...
						txtElementInfo.setText(SphincsDescriptions.ZestLabelProvider_5 + " " + n.getLeafNumber() + " = "
								+ n.getNameAsString()); // content of the node HORST
						...
						// content of the WOTS node
						txtElementInfo.setText(SphincsDescriptions.ZestLabelProvider_6 + " = " + n.getNameAsString());
	...
	private void generatSphincsTree() {
		...
		for (int layerCount = 0; layerCount < wholeTreeHeight - 1; layerCount++) {
			...
			for (int index = 0; index < singleTreeHeight; index++, treeIndex += 2) {
				if (layerCount == 0 && index == 0) { // index 0 lowest --> build HORST tree
					// SimpleNode(content, left leaf, right leaf)
					WotsUp = new SimpleNode(treeData[treeIndex], tree.get(treeIndex), tree.get(treeIndex + 1));
					tree.add(WotsUp);

					// Connect, index (e.g. treeLayer 0 left, with 0 and right with 1 of tree)
					...
					// setParent-Node
					tree.get(treeIndex).setParent(tree.get(tree.size() - 1)); // e.g. treeIndex = 0 := leaf .setParent
																				// treeLevel := no leaf
					tree.get(treeIndex + 1).setParent(tree.get(tree.size() - 1));
				}
			}
		...
		tree.get(tree.size() - 1).getConnectedTo().add(tree.get(treeIndex)); // connect element before last element hit
		tree.get(treeIndex).setParent(tree.get(tree.size() - 1));
	}
