	...
	private CertificateCSRR(){
		...
		if(!certsExist){
			...
			for(int i = 0; i<5; i++){
				...
				caKeys.add(keypair);
				certs.add(cert);
				KeyStoreAlias pubAlias = new KeyStoreAlias("JCT-CA Root Certificates - DO NOT DELETE", KeyType.KEYPAIR_PUBLIC_KEY, "RSA", 1024, kp.getPublic().hashCode()+"", kp.getPublic().getClass().toString());
				KeyStoreAlias privAlias = new KeyStoreAlias("JCT-CA Root Certificates - DO NOT DELETE", KeyType.KEYPAIR_PUBLIC_KEY, "RSA", 1024, kp.getPrivate().hashCode()+"", kp.getPrivate().getClass().toString());
