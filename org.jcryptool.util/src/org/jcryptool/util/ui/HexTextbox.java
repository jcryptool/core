...
public class HexTextbox extends Text {
	...
	public HexTextbox(Composite parent, int style, int numBytes, char defaultValue) {
		...
		errorToolTip.setText(Messages.getString("HexTextbox.ErrorToolTip.Title")); //$NON-NLS-1$
		errorToolTip.setMessage(Messages.getString("HexTextbox.ErrorToolTip.Message")); //$NON-NLS-1$
	 ...
	 * Disable their functionality, so that no one can mixes the correct behaviour up
	 */
	@Override
	...
	public byte[] getBytes() {
		...
		for(int i=0;i<bytes.length; i++)
			bytes[i] = (byte) ((Character.digit(text[2*i], 16) << 4)
					+ Character.digit(text[2*i+1], 16));
		...
		public void keyPressed(KeyEvent e) {
			...
			if(e.keyCode == SWT.ARROW_LEFT)
				...
				setCaretPosition(getCaretPosition()+1, false, false);
					...
					|| (e.character >= 32 && e.character <= 126))
				showErrorToolTip();
		}
