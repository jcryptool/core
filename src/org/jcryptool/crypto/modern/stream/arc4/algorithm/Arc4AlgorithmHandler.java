...
public class Arc4AlgorithmHandler extends AbstractAlgorithmHandler {
	...
	public Object execute(ExecutionEvent event) {
		...
		WizardDialog dialog = new WizardDialog(getActiveWorkbenchWindow()
				.getShell(), wizard);
		...
		if (result == Window.OK) {
			...
			algorithm.init(editorContent, keyArray, wizard.getAlgoIsArc4());
	 ...
	 * <P>
	 * same method is used in the wizard to perform the change from hex to
	 * binary when users swaps between these two
	 * 
	 * @param key
	 *            -> the key string
	 * @return -> byte array
	...
	/**
	 * 
	 * creats a Byte Array out of a hex-string.
	 * 
	 * @param s
	 * @return
	 */
