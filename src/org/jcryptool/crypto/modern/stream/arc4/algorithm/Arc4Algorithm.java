...
public class Arc4Algorithm extends AbstractModernAlgorithm {
	 ...
	 *
	 * @param input
	 *            the InputStream of the input file editor
	 * @param key
	 *            the key
	 */
	public void init(InputStream input, byte[] key, boolean algo) {
	...
	public IDataObject execute() {
		...
		while (readFromStream != -1) {
			...
			try {
				...
				dataObject.setOutputIS(new BufferedInputStream(
						new ByteArrayInputStream(
								((ByteArrayOutputStream) cipherOutputStream)
										.toByteArray())));
				dataObject
						.setOutput(((ByteArrayOutputStream) cipherOutputStream)
								.toByteArray());
