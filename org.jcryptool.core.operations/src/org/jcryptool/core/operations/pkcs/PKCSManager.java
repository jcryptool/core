...
public class PKCSManager {
	...
	public synchronized static PKCSManager getInstance() {
		if (instance == null) instance = new PKCSManager();
		return instance;
	}
			...
			if (extensions[i].getNamespaceIdentifier().equals("org.jcryptool.crypto.modern.pkcs7")) {				 //$NON-NLS-1$
				...
				for (int j=0; j < configElements.length; j++) {
					if (configElements[j].getName().equals("pkcsFactory")) {						 //$NON-NLS-1$
						try {
//							jctDefaultHandler = (AbstractKeyStoreHandler)configElements[j].createExecutableExtension("keyStoreHandler");
						    ...
						    LogUtil.logError(OperationsPlugin.PLUGIN_ID, "Exception while loading a keystore handler", e, false); //$NON-NLS-1$
						}
					}
				}
			}
