...
/**
 * The AlphaConverter class provides the function to convert char arrays to int
 * arrays and the way back, by using a specified alphabet. For indexing the
 * input values the class uses a hash map.
 *
 * @author amro
 * @version 0.1
 */
public class AlphaConverter {
    ...
    /**
     * Builds the alphabet by analyzing the alpha entry and after that the
     * alpha-chars are mapped.
     *
    ...
    public AlphaConverter(char[] alphaEntry) {
        ...
        try {
            ...
            LogUtil.logError(OperationsPlugin.PLUGIN_ID, "Exception while building the hashmap for an alphabet", e, false); //$NON-NLS-1$
        }
     ...
     *
     * @param input the int values to be converted back to the appropriate
     *        chars.
    ...
    /**
     * This method filters the non alpha chars. It is used to get the the cipher
     * input for classic algorithms out of the plain text.
     *
     * @param plain the plain text
     * @return the filtered plain text (cipher input)
     * @throws Exception an exception is thrown if the final buffer has no
     *         array.
     */
     ...
     * @throws Exception if a char is passed more than once in the alphabet
     *         option's parameter
     */
