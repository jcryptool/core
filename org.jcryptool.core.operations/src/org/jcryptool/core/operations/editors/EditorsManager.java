...
/**
 * The EditorsManager class is a singleton. It provides a global point of access
 * to the registered editor services. The class has only instance. The
 * registered editor services are collected in a hash map. The keys of the hash
 * map are the IDs of the editors and the appropriate values are the editor
 * services.
 *
...
public class EditorsManager {
	...
	/**
	 * Editor services, editorID presents the key and editorService the
	 * appropriate value.
	 */
	...
	/**
	 * Returns an input stream of the active editor content, or
	 * <code>null</code> if no editor is active.
	 *
	 * @return An input stream of the active editor content, or
	 *         <code>null</code> if no editor is active
	 */
	...
	public InputStream getContentInputStream(IEditorPart editorPart) {
		...
		return editorServices.get(editorPart.getEditorSite().getId())
				.getContentOfEditorAsInputStream(editorPart);
	}

	/**
	 * This method performs the retrieval of the (byte[]) content of the
	 * parameterized editor.
	 *
	 * @deprecated Do not use with regard to modern cryptography. Use streams
	 *             instead.
	 * @param editorPart
	 *            the editor the content is to be retrieved
	...
	public byte[] getContentAsBytes(IEditorPart editorPart) {
		...
		return editorServices.get(editorPart.getEditorSite().getId())
				.getContentOfEditorAsBytes(editorPart);
	}

	/**
	 * This method performs the retrieval of the (String) content of the
	 * parameterized editor.
	 *
	 * @deprecated Do not use with regard to modern cryptography. Use streams
	 *             instead.
	...
	public String getContentAsString(IEditorPart editorPart) {
		...
		return editorServices.get(editorPart.getEditorSite().getId())
				.getContentOfEditorAsString(editorPart);
	}
	...
	private void retrieveEditorServices() {
		...
		IExtensionPoint extensionPoint = Platform.getExtensionRegistry()
				.getExtensionPoint(OperationsPlugin.PLUGIN_ID,
	 ...
	 *             if no active workbench page could be found.
	 */
	 ...
	 * @return the streamed content of the editor; or <code>null</code> if no
	 *         editor with this title is available.
	 * @throws EditorNotFoundException
	 *             if no active workbench page could be found.
	 */
	public InputStream getEditorContent(final String title)
			throws EditorNotFoundException {
		IWorkbenchPage[] pages = PlatformUI.getWorkbench()
				.getActiveWorkbenchWindow().getPages();
	...
	/**
	 * @return an observable object which notifies the Observer every time the
	 *         "active editor" predicate has been given from one editor to
	 *         another.
	 */
	...
	/**
	 * @return an observable object which notifies the Observer every time there
	 *         are changes in the availability of editors; precise: every time
	 *         the set of accessible editors changes.
	 */
