...
/**
 * Die Klasse stellt das Feld mit der Funktionalitaet beim Tutorial zur Kryptanalyse von Transpositionschiffren mit
 * Hilfe eines Ameisenalgorithmus dar. Mit ihrer Hilfe ist es dem Anwender moeglich die Parameter des Algorithmus zu
 * veraendern und so interaktiv auf die Ablaeufe einzuwirken.
 *
...
public class Func extends Composite implements Observer {
    ...
    /**
     * Konstruktor. Erhaelt das Model, das die Daten des Tutorials verwaltet und das Composite an das das Func-Objekt
     * angehaengt werden soll.
     *
     * @param model Model des Tutorials
     * @param c Parent
     */
    ...
    /**
     * Update-Methode reagiert auf Aenderungen im Model und passt die Funktionalitaet daran an, an welcher Stelle sich
     * der Anwender innerhalb des Tutorials befindet.
     */
    ...
    private void step1() {
        if (nr == 0) {
            ...
            groupGeneral.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
            ...
            Group groupSteps = new Group(this, SWT.NONE);
            ...
            selectSingle.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 2, 1));
            ...
            selectMultiple.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 2, 1));
            ...
            repeat.addListener(SWT.Selection, new Listener() {
                public void handleEvent(Event e) {
                    if (!m.getWorking()) {
                        ...
                        // emulates the button clicks at the end of the animation
            ...
            groupSettings.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
            ...
            alphaslider.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
    ...
    private void step0() {
        if (nr != 0) {
            ...
            groupEncrypt.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
            ...
            txtperm.addKeyListener(new KeyListener() {
                public void keyPressed(KeyEvent e) {
                    // not too long
                    if ((txtperm.getText().length() >= 2 * slider.getSelection() - 1) && e.character != '\b')
                        e.doit = false;
                    // number already set
                    if (Character.isDigit(e.character) && txtperm.getText().contains("" + e.character)) //$NON-NLS-1$
                        e.doit = false;
                    // number out of range
                    if (Character.isDigit(e.character) && (Integer.parseInt("" + e.character) > m.getSize() //$NON-NLS-1$
            ...
            Group groupAnalyse = new Group(this, SWT.NONE);
            ...
            final Label labelSlider = new Label(groupAnalyse, SWT.NONE);
            ...
            sliderAnalyse.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
            sliderAnalyse.setEnabled(false);
        ...
        if (m.getText().equals("")) //$NON-NLS-1$
            ...
            if (selectAnalyse.getSelection()) {
                if (txtCipher.getText().length() == 0
                        || txtCipher.getText().length() % sliderAnalyse.getSelection() != 0)
                    weiter.setEnabled(false);
                else
                    weiter.setEnabled(true);
            } else
