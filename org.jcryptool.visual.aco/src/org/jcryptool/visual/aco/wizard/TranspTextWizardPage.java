...
public class TranspTextWizardPage extends WizardPage {
	...
	public static class PageConfiguration {
		...
		public PageConfiguration(TextInputWithSource text, Integer columnCount,
				Boolean crop, Integer cropLength,
		...
		public String getReoderedText() {
			return reorderedText;
		}


		public boolean equalsNeglectingTextsource(PageConfiguration c) {
			return getColumnCount().equals(c.getColumnCount())
					&& getCropLength().equals(c.getCropLength())
					&& getText().getText().equals(c.getText().getText())
					&& isCrop().equals(c.isCrop());
		}
	 ...
	 * 
	 **/
	public TranspTextWizardPage(int blockLength) {
	...
	public final void createControl(final Composite parent) {
...

		{
			...
			grpText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
					false));
			{
				compTextInputMode = new Composite(grpText, SWT.NONE);
				compTextInputMode.setLayoutData(new GridData(SWT.FILL,
				...
				lblLadenDesTextes.setLayoutData(new GridData(SWT.LEFT, SWT.TOP,
						true, false, 1, 1));
				lblLadenDesTextes.setBounds(0, 0, 55, 15);
				lblLadenDesTextes
						.setText(Messages.TranspTextWizardPage_lblLadenDesTextes_text);
				{
					...
					composite.setLayout(new GridLayout(1, false));
					{
						...
						btnJcteditor
								.setText(Messages.TranspTextWizardPage_btnJcteditor_text);
					}
					{
						comboEditorInputSelector = new Combo(composite,
								SWT.NONE) {
							@Override
							...
							public org.eclipse.swt.graphics.Point computeSize(
								...
								Point result = super.computeSize(wHint, hHint,
										changed);
								return new Point(getAppropriateXValue(result.x,
										160), result.y);
							};

							private int getAppropriateXValue(int superXCalc,
									int maxSize) {
								return Math.min(superXCalc, maxSize);
							}
						};
						GridData gd_comboEditorInputSelector = new GridData(
								SWT.FILL, SWT.CENTER, true, false, 1, 1);
						gd_comboEditorInputSelector.horizontalIndent = 15;
						comboEditorInputSelector
								.setLayoutData(gd_comboEditorInputSelector);
					}
					{
					...
					{
						compFileInputDetails = new Composite(composite,
								SWT.NONE);
						GridData gd_compFileInputDetails = new GridData(
								SWT.FILL, SWT.CENTER, false, false, 1, 1);
						gd_compFileInputDetails.horizontalIndent = 15;
						compFileInputDetails
								.setLayoutData(gd_compFileInputDetails);
						GridLayout gl_compFileInputDetails = new GridLayout(2,
								false);
						...
						lblFilenametxt = new Label(compFileInputDetails,
								SWT.NONE);
						lblFilenametxt.setBounds(0, 0, 55, 15);
						lblFilenametxt
								.setText(Messages.TranspTextWizardPage_lblFilenametxt_text);

						linkChangeFile = new Link(compFileInputDetails,
								SWT.NONE);
						linkChangeFile
								.setText(Messages.TranspTextWizardPage_link_text);
					}
					{
						btnZwischenablageeigeneEingabe = new Button(composite,
								SWT.RADIO);
						...
						btnZwischenablageeigeneEingabe
								.addSelectionListener(new SelectionAdapter() {
									...
									public void widgetSelected(SelectionEvent e) {
										if (btnZwischenablageeigeneEingabe
												.getSelection()) {
											doTxfieldActionPreservingVisibleLines(
													new Runnable() {
														@Override
														public void run() {
															txtInputText
			...
			}
			{
				...
				grpTextinputCtrls.setText(Messages.TranspTextWizardPage_text);
				{
					...
					{
						txtInputText = new Text(textfieldComp, SWT.MULTI
		...
		}
		{
			...
			{
				{
					parttextCheck = new Button(parttextGroup, SWT.CHECK
							| SWT.LEFT);
					parttextCheck
				...
				}
				{
					...
					parttextCount.setLayoutData(new GridData(SWT.BEGINNING,
				...
				}
				{
					...
					lblCharacters
							.setText(Messages.TranspTextWizardPage_lblCharacters_text);
				}
				{
					lblIfTheText = new Label(parttextGroup, SWT.WRAP);
					lblIfTheText
							.setText(Messages.TranspTextWizardPage_lblIfTheText_text);
					GridData lblIfthetextLData = new GridData(SWT.FILL,
					...
					lblIfTheText.setFont(new Font(Display.getCurrent(),
							fontdata));
				}
			}
		}
		{
			...
			}
			{
				...
				{
					GridData gd_blocklengthSpinner = new GridData(SWT.LEFT,
							SWT.CENTER, false, false, 1, 1);
					gd_blocklengthSpinner.heightHint = 18;
					blocklengthSpinner.setLayoutData(gd_blocklengthSpinner);
				}
				blocklengthSpinner.setMinimum(3);
				blocklengthSpinner.setMaximum(9);
	...
	/**
	 * Runs a runnable, which executes some code. The top visible line number of
	 * the text field is remembered, and after the runnable has finished, the
	 * top visible line is restored is set to the remembered number.
	 * 
	 * @param runnable
	 */
	protected void doTxfieldActionPreservingVisibleLines(
			final Runnable runnable, Text textfield) {
	...
	private void loadEditorsAndFillEditorChooser() {
		...
		for (int i = 0; i < editorRefs.size(); i++) {
			...
			if (activeReference != null
					&& activeReference.equals(editorRefs.get(i))) {
	...
	protected void setTextInputUIState(TextInputWithSource text,
			boolean writeText) {
		...
		btnZwischenablageeigeneEingabe.setSelection(sourceType
				.equals(TextSourceType.USERINPUT));
		if (writeText) {
			if (textInput != null) {
		...
		showLoadFromEditorComponents(sourceType
				.equals(TextSourceType.JCTEDITOR));
		if (sourceType.equals(TextSourceType.JCTEDITOR)) {
			// TODO:
			comboEditorInputSelector.select(editorRefs
	...
	private void showLoadFromFileComponents(boolean b, String fileName) {
		if (b) {
			...
			pageComposite.layout(new Control[] { lblFilenametxt,
					linkChangeFile, compFileInputDetails });
		} else {
			GridData ldata = (GridData) compFileInputDetails.getLayoutData();
			ldata.exclude = !b;
			compFileInputDetails.setVisible(b);
	...
	private void showLoadFromEditorComponents(boolean b) {
		if (b) {
			GridData ldata = (GridData) comboEditorInputSelector
					.getLayoutData();
			ldata.exclude = !b;
	...
	private void preview() {
		...
		if (lastPreviewedPageConfig != null) {
			if (!getPageConfiguration().equalsNeglectingTextsource(
					lastPreviewedPageConfig)) {
	...
	private void forcePreview() {
		...
		transpositionTable1.setReadInOrder(iniDirection);
		...
		transpositionTable1.setText(is, pageConfig.getColumnCount(),
				!pageConfig.isCrop(), pageConfig.getCropLength());
	...
	public PageConfiguration getPageConfiguration() {
		return new PageConfiguration(textInput.getContent(),
				blocklengthSpinner.getSelection(),
				parttextCheck.getSelection(), parttextCount.getSelection(),
				transpositionTable1.getText());
	}
