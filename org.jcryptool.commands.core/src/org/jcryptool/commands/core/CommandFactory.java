...
public class CommandFactory {
    static public List<Command> loadExtensions() {
        ...
        IExtensionPoint extensionPoint = registry
                .getExtensionPoint("org.jcryptool.commands.core.commands"); //$NON-NLS-1$
	...
	public static List<Command> loadUniqueExtensions() {
		...
		for(Command command: commandsToRemove) commands.remove(command);
		
		return commands;
	}

	private static boolean isExtendedCommand(
			IConfigurationElement iConfigurationElement) {
		if(iConfigurationElement.getChildren().length != 0) return true;
		return false;
	}
}
