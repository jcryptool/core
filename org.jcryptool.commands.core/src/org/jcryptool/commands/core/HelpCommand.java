...
public class HelpCommand extends AbstractCommand {
    ...
    public static Options createStaticOptions() {
        ...
        options.addOption(OptionBuilder.withLongOpt(EXAMPLES_LONG_OPT).hasArg(false) //$NON-NLS-1$
				.isRequired(false).withDescription( //$NON-NLS-1$
						Messages.HelpCommand_show_examples_label).create(
						EXAMPLES_SHORT_OPT));

        options.addOption(OptionBuilder.withLongOpt(COMMANDLIST_LONG_OPT).hasArg(false) //$NON-NLS-1$
				.isRequired(false).withDescription(Messages.HelpCommand_listcommandsLabel).create(
						COMMANDLIST_SHORT_OPT));
        ...
        try {
    		if(commandLine.hasOption(EXAMPLES_SHORT_OPT)) {
            	...
            	if (commandLine.getArgs().length == 1) {
                	...
                	if(commandLine.getArgs().length > 1) throw new ParseException(Messages.HelpCommand_tooManyArgs);
                	if(commandLine.getArgs().length < 1) throw new ParseException(Messages.HelpCommand_tooFewArgs);
                }
     ...
     * @param nameOfCommand the name of the command the examples are for (specify the name that was specified in the command line)
     */
    ...
    /**
     * This function returns the help for calls of this pattern:
     *   HELP cmd
	 *   ?? cmd
	 *   cmd HELP
	 *   cmd ??
...
//		    HelpFormatter formatter = new HelpFormatter();
//		    StringWriter writer = new StringWriter();
//		    formatter.printUsage(new PrintWriter(writer), CONSOLE_WIDTH, "vigenere", command.createOptions());
//		    formatter.printHelp(new PrintWriter(writer), CONSOLE_WIDTH, command.getCommandSyntax(), command
//		            .getDescription(), command.createOptions(), 4, 20, Messages.HelpCommand_0, false);
//		    return writer.toString();
		} else {
...
//		    return writer.toString();
    ...
    /**
     * returns the help that appaers when a command was called
     * with a bad combination of options/arguments
     *
     ...
     */
    public static Object getHelpForBadCommandCall(String commandName,
			Command command, String commandCallLine, ParseException e) {
    ...
    private static String getRefForAliases(Set<String> aliases) {
		if(aliases.size() == 0) throw new IllegalArgumentException("No aliases found"); //$NON-NLS-1$
	...
	/**
	 * Returns the reference to more help that appears in the help that
	 * appears when a command was called with a bad combination of options/arguments
	 */
	private static String getRefStringToFurtherHelpBadCommandCall(
			String commandName, Command command, String commandCallLine,
			ParseException e) {
		StringBuilder maskBuilder = new StringBuilder();
		List<String> maskArgs = new LinkedList<String>();
	...
	/**
     * Returns the part of the help that points out how to reach examples for the usage of the command.
     * Assumes that there are actually examples.
     *
     * @param commandName the name of the command
     */
     ...
     */
    private static String getRefStringToDetailedHelp(String commandName,
			Command command, String commandCallLine) {
     ...
     */
    private static String getDetailedHelpCmdlineFromShortHelpCmdline(
			String commandCallLine) {
		/*
		 * possible commandCallLines:
		 *   help cmd
		 *   ? cmd
		 *   cmd help
		 *   cmd ?
	...
	static List<Example> getExamples(String commandName, Command command) {
		return command instanceof ExtendedHelpCommand ? ((ExtendedHelpCommand) command).getExamples() : new LinkedList<ExtendedHelpCommand.Example>();
	}

	/**
	 * Returns aliases for a command, if it has any, and if not, returns an empty List.
	 * Aliases are, in this case, all names for the command that are NOT equal to the given command name.
	 */
	...
	/**
	 * Recovers the command line form of which a CommandLine object could've originated from.
	 * Assumes, that all options have been stated in short form.
	 *
	 * @param commandName the name of the command
