...
public class StartHandler extends AbstractHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		if (!view.hasContent()) {
			MessageDialog.openInformation(HandlerUtil.getActiveShell(event),
					Messages.StartHandler_0, Messages.StartHandler_1);
		}
		else {
			...
			if (start){
				...
				try {
					...
					if (!executed) {
						...
						for (RegistryType rt : RegistryType.values()) {
							if (rt.getName().equals(a.getPluginId())) {
								...
								if (a.getParam("algorithm type").equals(RegistryType.SECURE_RANDOM.getName())) { //$NON-NLS-1$
					...
					if (ActionsUIPlugin.getDefault().getPreferenceStore()
						...
						display.asyncExec(new Runnable() {
							public void run() {
					        	MessageDialog.openError(shell, Messages.StartHandler_3, e.getMessage()+Messages.StartHandler_4); //$NON-NLS-1$
							}
						});
						return Status.CANCEL_STATUS;
					}
	...
	public IDataObject convert(ActionItem actionItem) {
		...
		if (dataobject instanceof ClassicDataObject) {
			AbstractAlphabet alphabet = AlphabetsManager.getInstance().getAlphabetByName(
					actionItem.getAlphabet());
