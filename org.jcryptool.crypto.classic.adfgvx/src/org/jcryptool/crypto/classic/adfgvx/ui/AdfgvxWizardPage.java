...
public class AdfgvxWizardPage extends AbstractClassicCryptoPage {
	...
	private Label hALabel; private Label hDLabel; private Label hFLabel; private Label hGLabel; private Label hVLabel; private Label hXLabel;

	// the vertical name fields
	private Label vALabel; private Label vDLabel; private Label vFLabel; private Label vGLabel; private Label vVLabel; private Label vXLabel;

	// the matrix cells
	private Label rc11; private Label rc12; private Label rc13; private Label rc14; private Label rc15; private Label rc16;
	private Label rc21; private Label rc22; private Label rc23; private Label rc24; private Label rc25; private Label rc26;
	private Label rc31; private Label rc32; private Label rc33; private Label rc34; private Label rc35; private Label rc36;
	private Label rc41; private Label rc42; private Label rc43; private Label rc44; private Label rc45; private Label rc46;
	private Label rc51; private Label rc52; private Label rc53; private Label rc54; private Label rc55; private Label rc56;
	private Label rc61; private Label rc62; private Label rc63; private Label rc64; private Label rc65; private Label rc66;
	...
	private char[] fromCharacters(List<Character> chars) {
		...
		for(int i=0; i<chars.size(); i++) substitute[i] = chars.get(i);
		return substitute;
	}
	private List<Character> toCharacters(char[] chars) {
		List<Character> substitute = new LinkedList<Character>();
		for(int i=0; i<chars.length; i++) substitute.add(chars[i]);
		return substitute;
	}
	 ...
	 *   (Messages.AdfgvxWizardPage_onlyoccuronce)
	 * * RESET: einfach aus "" den standardkey umwandeln..
	 * 		resetFlag = true;
			keyWordText.setText(""); //$NON-NLS-1$
			resetFlag = false;
	...
	protected void createKeyInputObjects() {
		substitutionKeyInput = new KeyInput<List<Character>>() {
			...
			public void writeContent(List<Character> content) {
				if(content == null) setTextfieldTextExternal(""); //$NON-NLS-1$
				else {
			...
			protected InputVerificationResult verifyUserChange() {
				List<KeyVerificator> verificators = ((AdfgvxAlgorithmSpecification) specification).getKeyVerificatorsSubstitutionKey();
			...
			public List<Character> readContent() {
				...
				return toCharacters(factory.getCipherAlphabet(adfgvxAlphabet, getTextfield().getText().toUpperCase().toCharArray()));
			}
			@Override
		...
		transpositionKeyInput = new KeyInput<String>() {
			@Override
			protected InputVerificationResult verifyUserChange() {
				List<KeyVerificator> verificators = ((AdfgvxAlgorithmSpecification) specification).getKeyVerificatorsTranspositionKey();
				return KeyVerificator.verify(getTextfield().getText(), alphabetInput.getContent(), verificators);
			}
			@Override
	...
	public void labelsSetText(char[] cAlph) {
		...
		if (i < y) {
			...
			rc11.setText(new Character(cAlph[i]).toString());
			rc11.redraw();
			i++;
		}
		if (i < y) {

			rc12.setText(new Character(cAlph[i]).toString());
			rc12.redraw();
			i++;
		}
		if (i < y) {

			rc13.setText(new Character(cAlph[i]).toString());
			rc13.redraw();
			i++;
		}
		if (i < y) {

			rc14.setText(new Character(cAlph[i]).toString());
			rc14.redraw();
			i++;
		}
		if (i < y) {

			rc15.setText(new Character(cAlph[i]).toString());
			rc15.redraw();
			i++;
		}
		if (i < y) {

			rc16.setText(new Character(cAlph[i]).toString());
			rc16.redraw();
			i++;
		}
		if (i < y) {

			rc21.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc22.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc23.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc24.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc25.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc26.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc31.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc32.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc33.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc34.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc35.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc36.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc41.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc42.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc43.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc44.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc45.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc46.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc51.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc52.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc53.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc54.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc55.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc56.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc61.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc62.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc63.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc64.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {

			rc65.setText(new Character(cAlph[i]).toString());
			i++;
		}
		if (i < y) {
...
//		keyWordGroup.setText(Messages.AdfgvxWizardPage_keyword);
    ...
    protected void setHelpAvailable() {
        PlatformUI.getWorkbench().getHelpSystem().setHelp(getControl(),
                AdfgvxPlugin.PLUGIN_ID + ".wizard"); //$NON-NLS-1$
    }
