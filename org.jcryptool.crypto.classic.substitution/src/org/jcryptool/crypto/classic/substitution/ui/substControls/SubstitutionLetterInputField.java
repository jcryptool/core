package org.jcryptool.crypto.classic.substitution.ui.substControls;
...
//		UNDETERMINED_SUBST_COLOR = Display.getCurrent().getSystemColor(SWT.COLOR_DARK_RED);
	...
	private void initGUI(Mode mode, AbstractAlphabet alphabet) {
		...
		if(mode.hasMenu()) {
			...
			if(SubstitutionPlugin.getDefault() != null) {
				Image btnImage;
				if(mode == Mode.HORIZONTAL_POPUP) {
					btnImage = SubstitutionPlugin.getDefault().getImageRegistry().get(SubstitutionPlugin.TINY_ARROW_RIGHT);
				} else {
